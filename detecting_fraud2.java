...
public class practice {
	...
	public String[] detecting_fraud(String[] jobs){
		...
		HashMap<String,ArrayList<Integer>> start_index = new HashMap<String, ArrayList<Integer>>();
		...
		HashMap<String,ArrayList<Integer>> invoice_id = new HashMap<String,ArrayList<Integer>>();
		...
		for(int i=0;i<jobs.length;i++){
			...
			if(second_part.equals("START")){
				...
				//if this name is not in end_index hashmap, insert in start_index hashmap
						...
						ArrayList<Integer> temp_line_number = new ArrayList<Integer>();
						if(start_index.containsKey(name)) {
							temp_line_number = start_index.get(name);
							temp_line_number.add(line_number);
						}
						else{
							temp_line_number.add(line_number);
						}
						start_index.put(name,temp_line_number);
					}
			...
			//if 2nd part !=START i.e: numbers for invoice id and start_index still contains the name(i.e: it was not removed in logic above when 2nd part is START)
				...
				String[] second_part_string = second_part.split(",");
				ArrayList<Integer> second_part_int = new ArrayList<Integer>();
				for(int k=0;k<second_part_string.length;k++) {
					second_part_int.add(Integer.parseInt(second_part_string[k]));
				}
				//sort in ascending order for easy comparison in iterator loop
				Collections.sort(second_part_int);
				...
				while(it.hasNext()){
					Map.Entry<String,ArrayList<Integer>> pair = (Map.Entry)it.next();
					ArrayList<Integer> pairvalue = pair.getValue();
					//sort for easy comparison
					Collections.sort(pairvalue,Collections.reverseOrder());
					
					//if any invoice id > this invoice id, find start index for this name, find the end index for that found invoice id name.Because of proper sorting only comparing the 0th index for pairvalue and second_part_int will be enough.
					if(pairvalue.get(0)>second_part_int.get(0)){
						ArrayList<Integer> temp_start_index = start_index.get(name);
						...
						//for SHORTENED_JOB
						if(temp_start_index.size()==1){
						if(temp_start_index.get(0)>temp_end_index)	{
							String shortened_job = temp_start_index.get(0)+";"+name+";"+"SHORTENED_JOB";
							result_list.add(shortened_job);
						...
						//for SUSPICIOUS_BATCH						
						//if there are multiple invoice_id, current job is SUSPICIOUS_JOB. Remove this name from all 3 hashmaps.
						else{
							for(int j=0;j<temp_start_index.size();j++){
							if(temp_start_index.get(j)>temp_end_index)	{
							String suspicious_batch_job = line_number+";"+name+";"+"SUSPICIOUS_BATCH";
							result_list.add(suspicious_batch_job);
					...
					if(!flag_error){
						ArrayList<Integer> temp_invoice_number = new ArrayList<Integer>();
						temp_invoice_number.addAll(second_part_int);
						invoice_id.put(name,temp_invoice_number);
					}
	...
	public static void main(String[] args){
		...
		//String[] input_data = {"Nick;START","Jeremy;START","Leah;START","Nick;10","Jeremy;START","Jeremy;START","Leah;15","Jeremy;8,14,9"};
